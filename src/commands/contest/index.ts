	...
	handle: async () => {
		...
		try {
			...
			panel.webview.onDidReceiveMessage(async message => {
				console.log(
					`Got ${message.type} request: message = `,
					message.data
				);
				if (message.type === 'request-ranklist') {
					panel.webview.postMessage({
						type: 'ranklist',
						message: {
							html: await generateRanklist(
								res,
								await getRanklist(cid, message.data as number),
...
const generateRanklist = async (
	...
	for (
		...
		html += `<tr><td align="center" width="30px" nowrap>#${
			i + 50 * (nowpage - 1) + 1
		}</td><td align="center" style="font-weight: bold; color: ${getUsernameColor(
			ranklist['scoreboard']['result'][i]['user']['color']
		)};" nowrap>${
			ranklist['scoreboard']['result'][i]['user']['name']
		}${getUserSvg(
			ranklist['scoreboard']['result'][i]['user']['ccfLevel']
			...
			if (
				...
				if (
					...
					if (
						...
						if (
							ranklist['scoreboard']['result'][i]['user'][
								'uid'
							] ===
					...
					if (
						...
						if (
							...
							html +=
								' style="background-color: rgb(217, 240, 199);"';
						}
						html +=
    ...
    <body>
        ...
        <div data-v-6febb0e8="" data-v-72177bf8=""  class="card padding-default" style="margin-bottom: 1em;">
            <div data-v-83303c00="" class="field">
                ...
                <span data-v-83303c00="" class="value lfe-caption">${
	...
	if (contest['visibilityType'] > 3 && contest['visibilityType'] < 6) {
		html += `
                                <span data-v-360481bd="" class="wrapper">
                                    <span data-v-360481bd="" data-v-303bbf52="" style="font-weight: bold; color: ${getUsernameColor(
										contest['host']['color'].toLowerCase()
									)};">
                                        ${contest['host']['name']}
                                    </span>
	...
	html += `
                    ...
                    <div data-v-3a151854="">
                        ...
                        <span data-v-20b7d558 data-v-c0996248 class="lfe-caption tag" style="${
							contestStyle[contest['ruleType']]
						}">${contestType[contest['ruleType']]}</span>
                        <span data-v-20b7d558 data-v-c0996248  class="lfe-caption tag" style="${
							contestVisibilityStyle[contest['visibilityType']]
						}">${
                    ...
                    <div>
                        <span data-v-8d4c9aee="" class="lfe-caption">开始时间：${formatTime(
							new Date((contest['startTime'] as number) * 1000),
							'yyyy-MM-dd hh:mm:ss'
						)}</span>
                    </div>
                    <div>
                        <span data-v-8d4c9aee="" class="lfe-caption">结束时间：${formatTime(
							new Date((contest['endTime'] as number) * 1000),
							'yyyy-MM-dd hh:mm:ss'
						)}</span>
                    </div>
                    <div>
                        <span data-v-8d4c9aee="" class="lfe-caption">比赛时长：${changeTime(
        ...
        <script>
            ...
            document.getElementById("problem").innerHTML = showProblem(${JSON.stringify(
				res['contestProblems']
			)})
        </script>
        <!-- 以下为排行榜 -->
        ...
        function gotokthpage() {
            ...
            var tmp = parseInt(p);
            if (p === "" || tmp < 1 || tmp > Math.ceil(${
				ranklist['scoreboard']['count']
			} / 50.0)) {
