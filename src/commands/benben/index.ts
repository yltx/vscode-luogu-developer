	...
	if (mode === 'list') {
		for (let i = tmp.length - 1; i >= 0; i--) {
			...
			if (tmp[i].user.badge !== null && tmp[i].user.badge.length > 0) {
				badge = ` <span class="am-badge am-radius lg-bg-${tmp[
					i
				].user.color.toLowerCase()}">${tmp[i].user.badge}</span>`;
			}
			if (ccfLevel === 0) {
				message.push(`<li class="am-comment am-comment-primary feed-li"><div class="lg-left"><a href="https://www.luogu.com.cn/user/${
					tmp[i].user.uid
				}" class="center">
        <img src="data:image/jpeg;base64,${(
			await loadUserIcon(parseInt(tmp[i].user.uid))
		)?.toString('base64')}"
        class="am-comment-avatar"/></a></div><div class="am-comment-main"><header class="am-comment-hd"><div class="am-comment-meta"><span class="feed-username"><a class='${
			'lg-fg-' + tmp[i].user.color.toLowerCase() + ' lg-bold'
		}' href="https://www.luogu.com.cn/user/${
			tmp[i].user.uid
		}"  target="_blank">${
			tmp[i].user.name
		}</a>${badge}</span> ${benbenTime.toLocaleString()}<a name="feed-delete" data-feed-id="${
			tmp[i].id
		}">删除</a><a name="feed-reply" href="javascript: scrollToId('feed-content')" data-username="${
			tmp[i].user.name
		}">回复</a></div></header><div class="am-comment-bd"><span class="feed-comment">${md.render(
			tmp[i].content
		)}</span></div></div></li>`);
			} else {
				message.push(`<li class="am-comment am-comment-primary feed-li"><div class="lg-left"><a href="https://www.luogu.com.cn/user/${
        ...
        class="am-comment-avatar"/></a></div><div class="am-comment-main"><header class="am-comment-hd"><div class="am-comment-meta"><span class="feed-username"><a class='${
			...
			tmp[i].user.name
		}</a> <a class="sb_amazeui" target="_blank" href="https://www.luogu.com.cn/discuss/show/142324">${hook}</a>${badge}</span> ${benbenTime.toLocaleString()}<a name="feed-delete" data-feed-id="${
			tmp[i].id
		}">删除</a><a name="feed-reply" href="javascript: scrollToId('feed-content')" data-username="${
			tmp[i].user.name
		}">回复</a></div></header><div class="am-comment-bd"><span class="feed-comment">${md.render(
		...
		for (let i = tmp.length - 1; i >= 0; i--) {
			...
			if (userInfo.badge !== null && userInfo.badge.length > 0) {
				badge = ` <span class="am-badge am-radius lg-bg-${userInfo.color.toLowerCase()}">${
					userInfo.badge
				}</span>`;
			}
			if (ccfLevel === 0) {
        ...
        <img src="data:image/jpeg;base64,${(
			await loadUserIcon(parseInt(userInfo.uid))
		)?.toString('base64')}"
        class="am-comment-avatar"/></a></div><div class="am-comment-main"><header class="am-comment-hd"><div class="am-comment-meta"><span class="feed-username"><a class='${
			'lg-fg-' + userInfo.color.toLowerCase() + ' lg-bold'
		}' href="https://www.luogu.com.cn/user/${
			userInfo.uid
		}"  target="_blank">${userInfo.name}</a>${badge}</span> ${
			tmp[i].time.date.split('.')[0]
		}<a name="feed-reply" href="javascript: scrollToId('feed-content')" data-username="${
			userInfo.name
		}">回复</a></div></header><div class="am-comment-bd"><span class="feed-comment">${md.render(
			tmp[i].comment
		)}</span></div></div></li>`);
        ...
        class="am-comment-avatar"/></a></div><div class="am-comment-main"><header class="am-comment-hd"><div class="am-comment-meta"><span class="feed-username"><a class='${
			...
			userInfo.name
		}</a> <a class="sb_amazeui" target="_blank" href="https://www.luogu.com.cn/discuss/show/142324">${hook}</a>${badge}</span> ${
			tmp[i].time.date.split('.')[0]
		}<a name="feed-reply" href="javascript: scrollToId('feed-content')" data-username="${
			userInfo.name
		}">回复</a></div></header><div class="am-comment-bd"><span class="feed-comment">${md.render(
...
export default new SuperCommand({
	...
	handle: async () => {
		...
		panel.webview.onDidReceiveMessage(async message => {
			console.log(
				`Got ${message.type} request: message = ${message.data}`
			);
