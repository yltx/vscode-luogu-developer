...
md.renderer.rules.image = function (tokens, idx, options, env, self) {
    ...
    if (url.protocol == "bilibili:" && (aidRE.test(url.pathname) || bvidRE.test(url.pathname))) {
        let info: { aid?: string, bvid?: string, page: number, t: number }
            = { t: +(url.searchParams.get("t") || "0"), page: +(url.searchParams.get("page") || "1") };  // 若没有P号或时间标记，则默认为 0 与 1
        if (aidRE.test(url.pathname)) info.aid = (url.pathname[0] != 'a' ? 'av' : '') + url.pathname;  // 匹配 av 号。若匹配，将纯数字格式的 av 号开头也补上 "av"
