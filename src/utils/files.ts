...
export const initFiles = function (rootPath: string): [any, string] | null {
  ...
  try {
    if (!fs.existsSync(globalThis.luoguPath))
      fs.mkdirSync(globalThis.luoguPath);
  } catch (err) {
    return [err, '创建配置文件失败。'];
  }
  try {
    if (
      !fs.existsSync(globalThis.luoguJSONPath) ||
      !luoguJSONfileSchema(
        (configFile = JSON.parse(
          fs.readFileSync(globalThis.luoguJSONPath).toString()
        ))
      )
    )
      fs.writeFileSync(
        globalThis.luoguJSONPath,
        JSON.stringify(
          (configFile = {
            uid: '',
            clientID: '',
...
export const cookieConfig = function () {
  return {
    headers: {
      Cookie: `_uid=${configFile.uid};__client_id=${configFile.clientID}`
    }
  };
};
...
export const getSavedProblem = function (pid: string) {
  return fs
    .readFileSync(path.join(globalThis.luoguPath, pid + '.html'))
    .toString();
};
...
export const saveUserIconCache = function (uid: number, image: Buffer) {
  ...
  fs.writeFileSync(
    path.join(globalThis.luoguPath, `${uid}.savetime.txt`),
    new Date().getTime().toString()
  );
};
export const loadUserIconCache = function (uid: number) {
  if (
    fs.existsSync(path.join(globalThis.luoguPath, `${uid}.jpg`)) &&
    fs.existsSync(path.join(globalThis.luoguPath, `${uid}.savetime.txt`)) &&
    new Date().getTime() -
      parseInt(
        fs
          .readFileSync(path.join(globalThis.luoguPath, `${uid}.savetime.txt`))
          .toString()
      ) <=
      +vscode.workspace
